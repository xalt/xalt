...
const char * xalt_watermark() __attribute__((weak))
{
  return "FALSE";
}
  ...
  if ( run_mask & BIT_MPI)  
    {
	  ...
	  char * cmd2;
          asprintf(&cmd2, "LD_LIBRARY_PATH=%s PATH=%s %s --interfaceV %s --pid %d --ppid %d --syshost \"%s\" --start \"%.4f\" --end 0 --exec \"%s\" --ntasks %ld"
                   " --kind \"%s\" --uuid \"%s\" --prob %g --ngpus 0 --wm \"%s\" %s %s -- %s", CXX_LD_LIBRARY_PATH, XALT_SYSTEM_PATH, run_submission, XALT_INTERFACE_VERSION,
  ...
  else
    {
	  ...
	  char * cmd2    = NULL;
          ...
          asprintf(&cmd2, "LD_LIBRARY_PATH=%s PATH=%s %s --interfaceV %s --pid %d --ppid %d --syshost \"%s\" --start \"%.4f\" --end \"%.4f\" --exec \"%s\""
                   " --ntasks %ld --kind \"%s\" --uuid \"%s\" --prob %g --ngpus %d --wm \"%s\" %s %s -- %s", CXX_LD_LIBRARY_PATH, XALT_SYSTEM_PATH, run_submission,
