  ...
  if ( produce_strt_rec )
    {
      asprintf(&cmdline, "LD_LIBRARY_PATH=%s PATH=/usr/bin:/bin %s --ppid %d --syshost \"%s\" --start \"%.3f\" --end 0 --exec \"%s\" --ntasks %ld"
	       " --uuid \"%s\" %s %s -- %s %s", CXX_LD_LIBRARY_PATH, PREFIX "/libexec/xalt_run_submission", ppid, my_syshost,
...
void myfini()
{
  ...
  asprintf(&cmdline, "LD_LIBRARY_PATH=%s PATH=/usr/bin:/bin %s --ppid %d --syshost \"%s\" --start \"%.3f\" --end \"%.3f\" --exec \"%s\""
           " --ntasks %ld --uuid \"%s\" %s %s -- %s", CXX_LD_LIBRARY_PATH, PREFIX "/libexec/xalt_run_submission", ppid, my_syshost,
